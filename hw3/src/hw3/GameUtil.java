...
{  
	...
	public int getScoreForValue(int value)
	{
		...
		else
		{
			...
			score = Math.pow(3, score + 1);
			scoreValue = (int) score;					//converts the double value passed in the logarithm to an int
		}
	...
	{
		int position = rand.nextInt(grid.length - 1);
		int row = 0;
		int column = 0;
		int tileValue = 0;
		...
		TilePosition tile = new TilePosition(row, column, tileValue);
		return tile;
	}
	...
	public ArrayList<Move> shiftArray(int[] arr)
	{
		...
		for(int i = 0; i < arr.length - 1; i++)
		{
			if((arr[index] == 0) && (arr[index + 1] != 0))
			{
				...
				arr[index + 1] = 0;
				list.add(new Move(index + 1, index, arr[index + 1]));
			}
			else if(mergeValues(arr[index], arr[index + 1]) >= 3)
			{
				arr[index] = mergeValues(arr[index], arr[index + 1]);
				arr[index + 1] = 0;
				list.add(new Move(index + 1, index, arr[index + 1], arr[index], arr[index + 1] + arr[index]));
			}
			index++;
		}
		return list;
